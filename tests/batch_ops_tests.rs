...
fn test_batch_size<const N: usize>() {
    ...
    for i in 0..N {
        let expected = a_data[i] & b_data[i];
        assert_eq!(result_and.get(i).to_u128(), expected.to_u128(), 
                   "AND mismatch at index {} for size {}", i, N);
    }
    ...
    for i in 0..N {
        let expected = a_data[i] | b_data[i];
        assert_eq!(result_or.get(i).to_u128(), expected.to_u128(), 
                   "OR mismatch at index {} for size {}", i, N);
    }
    ...
    for i in 0..N {
        let expected = a_data[i] ^ b_data[i];
        assert_eq!(result_xor.get(i).to_u128(), expected.to_u128(), 
                   "XOR mismatch at index {} for size {}", i, N);
    }
}
...
fn test_combine_all() {
    ...
    let expected = SimdBitboard::from_u128(
        0x0000_0000_1111_0000_1111_0000_1111_1111
    );
...
fn test_combine_all_single() {
    let attacks = AlignedBitboardArray::<1>::from_slice(&[
        SimdBitboard::from_u128(0xAAAA),
    ]);
