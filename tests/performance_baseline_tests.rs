...
fn test_baseline_regression_detection() {
    ...
    let nodes_regression =
        result.regressions.iter().find(|r| r.metric == "nodes_per_second").unwrap();
    assert!(nodes_regression.change_percent < -5.0);

    let cutoff_regression =
        result.regressions.iter().find(|r| r.metric == "average_cutoff_rate").unwrap();
