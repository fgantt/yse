...
fn promoted_capture_converts_to_demoted_hand_value() {
    ...
    board_with_piece
        .place_piece(Piece::new(PieceType::PromotedBishop, Player::White), promoted_pos);
...
fn impasse_thresholds_respect_24_point_rule() {
    let mut board = BitboardBoard::empty();
    board.place_piece(Piece::new(PieceType::King, Player::Black), Position::new(1, 4));
    board.place_piece(Piece::new(PieceType::King, Player::White), Position::new(7, 4));

    // Black pieces
    board.place_piece(Piece::new(PieceType::Rook, Player::Black), Position::new(2, 2));
    board.place_piece(Piece::new(PieceType::Bishop, Player::Black), Position::new(2, 6));
    board.place_piece(Piece::new(PieceType::Gold, Player::Black), Position::new(1, 5));
    board.place_piece(Piece::new(PieceType::Silver, Player::Black), Position::new(1, 3));
    board.place_piece(Piece::new(PieceType::Knight, Player::Black), Position::new(1, 1));
    board.place_piece(Piece::new(PieceType::Lance, Player::Black), Position::new(1, 7));
    board.place_piece(Piece::new(PieceType::Pawn, Player::Black), Position::new(2, 4));

    // White pieces
    board.place_piece(Piece::new(PieceType::Rook, Player::White), Position::new(6, 6));
    board.place_piece(Piece::new(PieceType::Bishop, Player::White), Position::new(6, 2));
    board.place_piece(Piece::new(PieceType::Gold, Player::White), Position::new(7, 3));
    board.place_piece(Piece::new(PieceType::Silver, Player::White), Position::new(7, 5));
    board.place_piece(Piece::new(PieceType::Knight, Player::White), Position::new(7, 7));
    board.place_piece(Piece::new(PieceType::Lance, Player::White), Position::new(7, 1));
    ...
    let result = board.check_impasse_result(&captured).expect("impasse expected");
    ...
    let downgraded = board.check_impasse_result(&captured).expect("still impasse");
    assert!(downgraded.white_points < 24);
    assert_eq!(downgraded.outcome, shogi_engine::types::ImpasseOutcome::BlackWins);
}
...
fn statistics_reset_between_repeated_positions() {
    ...
    board_second.place_piece(Piece::new(PieceType::Pawn, Player::Black), Position::new(4, 4));
    ...
    assert_eq!(pawn_contrib.mg, evaluator.get_piece_value(PieceType::Pawn).mg as i64);
