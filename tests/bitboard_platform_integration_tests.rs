...
use shogi_engine::bitboards::{
    get_board_telemetry, get_magic_telemetry,
    integration::{BitScanningOptimizer, GlobalOptimizer},
    reset_board_telemetry, BitboardBoard,
};
...
fn test_platform_specific_optimizations() {
    ...
    let rook_attacks =
        board.get_attack_pattern(Position::new(4, 4), shogi_engine::types::PieceType::Rook);
    let bishop_attacks =
        board.get_attack_pattern(Position::new(4, 4), shogi_engine::types::PieceType::Bishop);

    // Should use optimized bit operations internally
    assert!(
        rook_attacks != SimdBitboard::new(0)
            || board.get_occupied_bitboard() != SimdBitboard::new(0)
    );
    ...
    {
        assert!(
            bishop_attacks != SimdBitboard::new(0)
                || board.get_occupied_bitboard() != SimdBitboard::new(0)
        );
    }
}
...
fn test_strategy_selection_telemetry() {
    ...
    let total_before = counters_before.popcount_hardware
        + counters_before.popcount_4bit
        + counters_before.popcount_swar
        + counters_before.popcount_debruijn
        + counters_before.bitscan_hardware
        + counters_before.bitscan_debruijn;

    let total_after = counters_after.popcount_hardware
        + counters_after.popcount_4bit
        + counters_after.popcount_swar
        + counters_after.popcount_debruijn
        + counters_after.bitscan_hardware
        + counters_after.bitscan_debruijn;
...
fn test_global_optimizer_platform_independence() {
    ...
    for (bb, expected_count) in test_cases {
        ...
        if bb != SimdBitboard::new(0) {
            ...
            assert!(
                first_bit.is_some(),
                "Bit scan forward should find bit in 0x{:X}",
                bb.to_u128()
            );
        }
    }
...
fn test_bitboard_operations_cross_platform() {
    ...
    use shogi_engine::types::{Player, Position};
    let center = Position::new(4, 4);
...
fn test_estimate_bit_count_accuracy() {
    ...
    for (bb, expected) in test_cases {
        ...
        assert_eq!(estimated, expected, "Estimate should match actual for 0x{:X}", bb);
    }
}
