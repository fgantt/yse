...
fn test_castle_progress_with_config() {
    ...
    config.castle_progress_bonus = TaperedScore::new(50, 30);
    config.castle_progress_penalty = TaperedScore::new(-30, -20);

    let evaluator = KingSafetyEvaluator::with_config(config);
    let _safety_score = evaluator.evaluate(&board, player, None);
...
fn test_storm_penalties_with_low_progress() {
    ...
    config.castle_progress_penalty = TaperedScore::new(-50, -30);
    ...
    let safety_score = evaluator.evaluate(&board, player, Some(&storm_state));
...
fn test_castle_progress_bonus() {
    ...
    config.castle_progress_bonus = TaperedScore::new(40, 25);

    let evaluator = KingSafetyEvaluator::with_config(config);
    let safety_score = evaluator.evaluate(&board, player, None);
...
fn test_storm_escalation() {
    ...
    let evaluator = KingSafetyEvaluator::new(config);
    ...
    if storm_state.total_severity > 0.0 {
        let castle_eval = evaluator.evaluate_castle(&board, player);
        let progress = castle_eval.progress_ratio();
        
        // If progress is below threshold, penalties should escalate with storm
        if progress < config.minimum_castle_progress {
            // Verify that storm penalties are applied
            let safety_score = evaluator.evaluate(&board, player, Some(&storm_state));
            // Score should reflect both low progress and storm pressure
        }
    }
}
...
fn test_castle_progress_decay() {
    ...
    let safety_no_storm = evaluator.evaluate(&board, player, None);
    ...
    if storm_state.total_severity > 0.0 {
        let safety_with_storm = evaluator.evaluate(&board, player, Some(&storm_state));
