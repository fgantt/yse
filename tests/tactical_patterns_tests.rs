...
fn test_simd_fork_detection_same_results() {
    ...
    assert!(result.mg.abs() < 100000 && result.eg.abs() < 100000,
...
fn test_simd_fork_detection_empty_board() {
    ...
    assert_eq!(result, shogi_engine::types::evaluation::TaperedScore::default(),
...
fn test_simd_pattern_matcher_direct_comparison() {
    ...
    assert!(simd_forks.len() <= pieces.len(),
           "Number of forks should not exceed number of pieces");
