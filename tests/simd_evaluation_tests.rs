...
fn test_evaluate_hand_material_batch() {
    ...
    let score = evaluator.evaluate_hand_material_batch(&captured_pieces, &piece_values, Player::Black);
    
    // With no captured pieces, score should be zero
    assert_eq!(score, TaperedScore::default(), "No captured pieces should have zero hand material score");
}
