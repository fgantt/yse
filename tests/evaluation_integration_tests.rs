...
fn test_simd_evaluation_same_results_as_scalar() {
    ...
    assert!(result.score.abs() < 100000,
               "PST evaluation should produce reasonable score (got {})", result.score);
...
fn test_simd_evaluation_empty_board() {
    ...
    assert_eq!(result.score, 0,
               "Empty board with only PST should have zero score");
}
...
fn test_simd_evaluation_with_pieces() {
    ...
    assert_ne!(result.score, 0,
               "Board with pieces should have non-zero PST score");
    ...
    if let Some(tel) = telemetry {
        if let Some(pst_tel) = tel.pst {
            assert!(!pst_tel.per_piece.is_empty() || pst_tel.total_mg != 0 || pst_tel.total_eg != 0,
...
fn test_simd_evaluation_player_switching() {
    ...
    assert_eq!(black_result.score, -white_result.score,
