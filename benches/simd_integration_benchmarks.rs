...
fn bench_simd_vs_scalar_evaluation(c: &mut Criterion) {
    ...
    group.bench_function("simd", |b| {
        b.iter(|| {
            black_box(evaluator_simd.evaluate(black_box(&board), Player::Black, black_box(&captured)));
        });
    });
    group.bench_function("scalar", |b| {
        b.iter(|| {
            black_box(evaluator_scalar.evaluate(black_box(&board), Player::Black, black_box(&captured)));
        });
    });
    group.finish();
}
...
fn bench_simd_vs_scalar_pattern_matching(c: &mut Criterion) {
    ...
    group.bench_function("simd", |b| {
        b.iter(|| {
            black_box(recognizer_simd.evaluate_tactics(black_box(&board), Player::Black, black_box(&captured)));
        });
    });
    group.bench_function("scalar", |b| {
        b.iter(|| {
            black_box(recognizer_scalar.evaluate_tactics(black_box(&board), Player::Black, black_box(&captured)));
        });
    });
    group.finish();
}
...
fn bench_simd_vs_scalar_move_generation(c: &mut Criterion) {
    ...
    group.bench_function("simd", |b| {
        b.iter(|| {
            black_box(generator_simd.generate_legal_moves(black_box(&board), Player::Black, black_box(&captured)));
        });
    });
    group.bench_function("scalar", |b| {
        b.iter(|| {
            black_box(generator_scalar.generate_legal_moves(black_box(&board), Player::Black, black_box(&captured)));
        });
    });
    group.finish();
}

criterion_group!(benches, bench_simd_vs_scalar_evaluation, bench_simd_vs_scalar_pattern_matching, bench_simd_vs_scalar_move_generation);
