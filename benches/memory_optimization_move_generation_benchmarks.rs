...
fn bench_move_generation_with_memory_optimizations(c: &mut Criterion) {
    ...
    for (name, board) in boards {
        ...
        group.bench_function(BenchmarkId::new("with_prefetching", name), |b| {
            b.iter(|| {
                let moves = move_generator
                    .generate_all_piece_moves(black_box(&board), black_box(Player::Black));
                black_box(moves)
            });
        });
    }
...
fn bench_sliding_move_generation_batch(c: &mut Criterion) {
    ...
    for (name, board) in boards {
        ...
        let magic_table =
            board.get_magic_table().unwrap_or_else(|| Arc::new(MagicTable::default()));
        ...
        group.bench_function(BenchmarkId::new("batch_vectorized", name), |b| {
            b.iter(|| {
