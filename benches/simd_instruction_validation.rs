...
/// 1. Build with: cargo build --release --features simd --bench
///    simd_instruction_validation
/// 2. Disassemble: objdump -d
///    target/release/deps/simd_instruction_validation-*.exe | grep -E
///    "(pand|por|pxor|andnot|vand|vorr|veor)" Or on macOS: otool -tv
///    target/release/deps/simd_instruction_validation-* | grep -E
///    "(pand|por|pxor|andnot|vand|vorr|veor)"
///
...
fn bench_simd_instructions(c: &mut Criterion) {
    ...
    // These operations should generate SIMD instructions when simd feature is
    // enabled
