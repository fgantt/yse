...
use shogi_engine::bitboards::{batch_ops::AlignedBitboardArray, platform_detection, SimdBitboard};
...
fn bench_batch_and(c: &mut Criterion) {
    ...
    group.bench_function("scalar_batch_and_16", |bencher| {
        bencher.iter(|| black_box(scalar_batch_and(&a_data, &b_data)));
    });
...
fn bench_batch_or(c: &mut Criterion) {
    ...
    group.bench_function("scalar_batch_or_16", |bencher| {
        bencher.iter(|| black_box(scalar_batch_or(&a_data, &b_data)));
    });
...
fn bench_batch_xor(c: &mut Criterion) {
    ...
    group.bench_function("scalar_batch_xor_16", |bencher| {
        bencher.iter(|| black_box(scalar_batch_xor(&a_data, &b_data)));
    });
...
fn bench_combine_all_size<const N: usize>(
    group: &mut criterion::BenchmarkGroup<'_, criterion::measurement::WallTime>,
) {
